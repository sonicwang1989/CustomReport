(function (window) {
    ...
    //判断两个对象的属性是否一样
    var objIsEqual = function (a, b, fields) {
        var isEqual = true;
    ...
    //数组分组
    var groupBy = function (array, field) {
        var groups = [];
        if (array && field) {
    ...
    //排序并返回新数组
    var sortBy = function (array, field, order) {
        var func = new Function('a', 'b', 'return a.' + field + ((order == 'asc') ? '>' : '<') + 'b.' + field + '?1:-1');
        return array.concat([]).sort(func);
    };

    //根据据本地排序
    var localeCompare = function (array, field, order) {
    ...
    var CustomReport = function () {
        ...
        this.GroupData = function () {
            ...
            array = localeCompare(array, "value", "asc");
            //添加合计行
        ...
        this.CreateColumnsTable = function () {
            ...
            var firstField = first(self.Cols);
            var lastField = last(self.Cols);
